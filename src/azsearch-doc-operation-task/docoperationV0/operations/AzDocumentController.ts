...
import { AZSEARCH_DATAAPI_VERSION, AZSEARCH_MGMTAPI_VERSION, AzureSearchClient, AdminKeyResult, DocumentOptions, DocumentOpResult } from '../../../common'
    ...
    return new Promise<DocumentOpResult[]>(async (resolve, reject) => {
      try{
        if(this.docOptions.payloadPath){
          this.docOptions.payload = fs.readFileSync(this.docOptions.payloadPath, 'utf8');
        }
        ...
        let request = await this.asClient.azureClient.sendRequest(options, (err, result: any, request, response) => {
          let opsList: DocumentOpResult[] = result.value;
            if (err) {
              ...
              console.log(tl.loc("AzureSearchDocumentSuccess", this.docOptions.azsearchName));
              resolve (opsList);
            } else if (response.statusCode==207) {
              console.log(tl.loc("AzureSearchDocumentWarning", this.docOptions.azsearchName));
              resolve (opsList);
            } else if (response.statusCode==429) {
              console.log(tl.loc("AzureSearchDocumentQuotaExceeded", this.docOptions.azsearchName));
              resolve (opsList);
            } else {
      ...
      try{
        ...
        let request = await this.asClient.azureClient.sendRequest(options, (err, result: any, request, response) => {
          if (err) {
            ...
            console.log(tl.loc("AzureSearchDocumentSearchExecuted", (result["@odata.count"])));
            resolve (result);
          }
        });
      } catch(error) { reject(error); }
